  ...
  qualification: z
    .object({
      cowType: z
        ...
        .default("")
        .describe(
          "Type of miniature cow (e.g., Nano Cow, Micro Miniature Cow, Punganuru Cow). Return empty string if not mentioned.",
        ),
      city: z
        .string()
        .default("")
      ...
      intent: z
        ...
        .default("")
        .describe(
          "Service user is interested in (Miniature Cow Sales, General Enquiry). Return empty string if not mentioned.",
        ),
      urgency: z
        .string()
        .default("Medium")
...
Latest Message: ${incomingText}`;
    ...
    const systemPrompt = `You are a friendly, human representative working at Minicow. Your goal is to convert leads, assist customers, provide support, and answer inquiries from potential buyers. NEVER mention that you are an AI or a bot. Act completely human.
...
- Cow Type: ${lead.aiQualification?.cowType || "Missing"}
...
7. PASSIVE EXTRACTION: Always extract 'Cow Type', 'City', and 'Intent' into the corresponding JSON fields if mentioned.
...
11. RESTRICTIONS: NEVER ask for Payment, Phone, Email, or OTP. Just ask for their City and exact requirement regarding miniature cows.
12. OUTPUT: Respond purely via the structured JSON schema.`;
    ...
    if (parsed.qualification) {
      ...
      updatePayload.aiQualification = {
        cowType: aiData.cowType || prevQual.cowType || "",
      ...
      if (!matchedService) {
        ...
        if (
          ...
          matchedService = "Miniature Cow Sales";
        }
      }
